...
const int cameraTriggerPin = 3;
unsigned long lastTriggerTime = 0;
const unsigned long triggerInterval = 10000;
...
void setup() {
  ...
  if (!bmp.begin_I2C(0x77, &Wire)) {  // Uses second pair of I2C pins for Teensy 4.1
    ...
    logFile.println("Failed to detect and initialize BMP388 on startup!");
    digitalWrite(redPin, HIGH);
    while (1);  // Infinite loop to stop the program
  }
  ...
  if (!imu.init() || !mag.init()) {
    logFile.println("Failed to detect IMU sensor on startup!");
    digitalWrite(redPin, HIGH);
